...
class BaseMouth:
    ...
    def say_multiple_stream(
        ...
        while True:
            text = text_queue.get()
            if "Filler:" in str(text) and Filler_Check == False:
                Filler_Check = True
                text = text.replace("Filler:", "")
                output = self.run_tts(text)
                audio_queue.put((output, text))
                all_response.append(text)
                continue

            if "Filler" in str(text) and Filler_Check == True:
                continue
        ...
        interrupt_text_list.append(response)
        audio_queue.put((None, ""))
        ...
        #                 first_audio = False
        #             else:
        #                 print(f"In run_tts putting to audio_que: {clean_sentence}")
        #                 output = self.run_tts(clean_sentence)
